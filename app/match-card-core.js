...
function fmtMove(value){if(value===null||value===undefined||value==='')return '';const n=Number(value);if(Number.isNaN(n))return '';const pp=Math.abs(n)<=1?n*100:n;return `${pp>0?'+':''}${pp.toFixed(2).replace(/\.00$/,'')} п.п.`}
...
function header(payload){
  ...
  return `<section class="match-card-v2-header"><div class="match-card-v2-league"><div>${leagueMedia}</div><span>${esc(f.round||'Тур не подтверждён')}</span></div><div class="match-card-v2-matchup"><div class="match-card-v2-team">${media(f.home?.logo_url,'match-card-v2-crest','Логотип хозяев')}<strong>${esc(f.home?.name||'Хозяева')}</strong>${redCards(f.home?.red_cards)}</div><div class="match-card-v2-center"><div class="match-card-v2-score">${center}</div><div class="match-card-v2-status">${esc(formatMatchCardStatus(status))}${liveMeta}</div></div><div class="match-card-v2-team away">${media(f.away?.logo_url,'match-card-v2-crest','Логотип гостей')}<strong>${esc(f.away?.name||'Гости')}</strong>${redCards(f.away?.red_cards)}</div></div><div class="match-card-v2-meta">${badge(`Freshness: ${freshness}`,freshnessTone)}${f.source_status?badge(f.source_status):''}${f.observed_at_utc?`<span>Снимок ${esc(fmtMoscow(f.observed_at_utc))}</span>`:''}${f.live_observed_at_utc?`<span>LIVE ${esc(fmtMoscow(f.live_observed_at_utc))}</span>`:''}</div></section>`;
}
...
function prediction(payload){const p=payload?.prediction||{};const items=list(p.items);if(!items.length)return section('Оценка ПБК',empty('Для этого матча нет валидированной probability-записи.'));
  return section('Оценка ПБК',`<div class="match-card-v2-list">${items.map(x=>`<div class="match-card-v2-row"><div><strong>${esc(x.rule||'R')} · ${esc(x.selection||'—')}</strong><small>${x.market_family?`${esc(x.market_family)} · `:''}${esc(x.model_version||'model')} · ${esc(x.status||'frozen prematch')}</small></div><div class="match-card-v2-probs"><span>Вероятность ПБК <b>${fmtPct(x.p_pbk)}</b></span><span>Рынок без маржи <b>${fmtPct(x.p_market_no_vig)}</b></span></div></div>`).join('')}</div><div class="match-card-v2-note">Вероятность ≠ value и не создаёт новый сигнал.</div>`);
}

function marketSelection(item={}){const meta=[];if(item.team_name)meta.push(item.team_name);if(item.line!==null&&item.line!==undefined&&item.line!=='')meta.push(`линия ${item.line}`);return `<div class="match-card-v2-market-selection"><div><strong>${esc(item.selection||'—')}</strong>${meta.length?`<small>${meta.map(esc).join(' · ')}</small>`:''}</div><div class="match-card-v2-market-prices">${item.best_odds?`<span>best <b>${fmtNum(item.best_odds)}</b>${item.best_book?` @ ${esc(item.best_book)}`:''}</span>`:''}${item.bet365_odds?`<span>Bet365 <b>${fmtNum(item.bet365_odds)}</b></span>`:''}${item.user_odds?`<span>${esc(item.user_bookmaker||'user')} <b>${fmtNum(item.user_odds)}</b></span>`:''}${item.market_no_vig!==null&&item.market_no_vig!==undefined&&item.market_no_vig!==''?`<span>no-vig <b>${fmtPct(item.market_no_vig)}</b></span>`:''}${item.movement_pp?`<span>движение <b>${esc(fmtMove(item.movement_pp))}</b></span>`:''}</div></div>`}
function markets(payload){const m=payload?.markets||{},families=list(m.families);if(!families.length)return section('Рынки',empty('Рыночные снимки для этой карточки пока не собраны.'));
  const available=families.filter(x=>x?.available);const unavailable=families.filter(x=>!x?.available);
  const cards=available.map(f=>`<div class="match-card-v2-market-family"><div class="match-card-v2-market-head"><div><strong>${esc(f.label_ru||f.id||'Рынок')}</strong><small>${esc(f.scope||'OBSERVATION_ONLY')}${f.source?` · ${esc(f.source)}`:''}</small></div><div>${f.research_only?badge('research','warn'):badge('market data')}${f.observed_at_utc?badge(fmtMoscow(f.observed_at_utc,false),'good'):''}</div></div><div class="match-card-v2-market-items">${list(f.items).map(marketSelection).join('')}</div>${f.no_quarter_lines?`<div class="match-card-v2-market-policy">Четвертные азиатские линии не выводятся.</div>`:''}</div>`).join('');
  const missing=unavailable.length?`<div class="match-card-v2-market-missing"><span>Нет подтверждённого pre-match снимка:</span>${unavailable.map(f=>badge(f.label_ru||f.id,'neutral')).join('')}</div>`:'';
  return section('Рынки',`<div class="match-card-v2-market-summary"><strong>Покрытие ${esc(m.available_family_count??available.length)}/${esc(m.total_family_count??families.length)}</strong>${badge('pre-match frozen','good')}<span>Рынок ≠ прогноз ПБК</span></div><div class="match-card-v2-market-grid">${cards||empty('Нет доступных рыночных семейств.')}</div>${missing}<div class="match-card-v2-note">Рыночные вероятности — только no-vig из сохранённых снимков. creates_signal=false · модель не изменяется.${m.user_line_policy==='NO_QUARTER_ASIAN_HANDICAPS'?' Четвертные азиатские форы скрыты.':''}</div>`,'markets');
}

function odds(payload){const items=list(payload?.odds?.items);if(!items.length)return section('Canonical odds / execution',empty('Коэффициенты canonical-сигнала для этой карточки пока не собраны.'));
  return section('Canonical odds / execution',`<div class="match-card-v2-list">${items.map(x=>`<div class="match-card-v2-row"><div><strong>${esc(x.rule||'R')} · ${esc(x.selection||'—')}</strong><small>${x.captured_at_utc?`Снимок ${esc(fmtMoscow(x.captured_at_utc,false))}`:'Снимок неизвестен'}</small></div><div class="match-card-v2-odds"><b>${fmtNum(x.best_odds)}</b><span>${esc(x.best_book||'—')}</span><small>user ${fmtNum(x.user_best_odds)} ${x.user_best_book?`@ ${esc(x.user_best_book)}`:''} · Bet365 ${fmtNum(x.bet365_odds)}</small></div></div>`).join('')}</div><div class="match-card-v2-note">Этот блок относится только к canonical execution, а не ко всему рынку.</div>`);
}

function fixtureMini(title,x={}){if(!x||(!x.opponent&&!x.fixture_id&&!x.date_utc))return `<div class="match-card-v2-mini"><span>${esc(title)}</span><b>Нет подтверждённых данных</b></div>`;return `<div class="match-card-v2-mini"><span>${esc(title)}</span><b>${esc(x.opponent||'Соперник не указан')}</b><small>${x.date_utc?esc(fmtMoscow(x.date_utc)):''}${x.competition?` · ${esc(x.competition)}`:''}${x.competition_class?` · ${esc(x.competition_class)}`:''}${x.hours_to_next!==null&&x.hours_to_next!==undefined?` · через ${esc(x.hours_to_next)} ч`:''}${String(x.is_uefa_or_cup||'').toUpperCase()==='YES'?' · UEFA/кубок':''}</small></div>`}
function context(payload){const c=payload?.context||{};if(!c.available)return section('Контекст матча',empty('Нет подтверждённых данных по контексту матча.'));
  const v=c.venue||{},rest=c.rest_hours||{},prev=c.previous_fixture||{},next=c.next_fixture||{};return section('Контекст матча',`<div class="match-card-v2-context-meta">${c.captured_at_utc?badge(`Context ${fmtMoscow(c.captured_at_utc,false)}`,'good'):''}${c.snapshot_type?badge(c.snapshot_type):''}${c.context_only?badge('context only'):''}</div><div class="match-card-v2-facts"><div><span>Судья</span><b>${esc(c.referee||'—')}</b></div><div><span>Стадион</span><b>${esc(v.name||'—')}</b><small>${esc(v.city||'')}</small></div><div><span>Отдых хозяев</span><b>${fmtNum(rest.home,' ч')}</b></div><div><span>Отдых гостей</span><b>${fmtNum(rest.away,' ч')}</b></div></div><div class="match-card-v2-two">${fixtureMini('Предыдущий матч · хозяева',prev.home)}${fixtureMini('Предыдущий матч · гости',prev.away)}${fixtureMini('Следующий матч · хозяева',next.home)}${fixtureMini('Следующий матч · гости',next.away)}</div>${c.notes?`<div class="match-card-v2-note">${esc(c.notes)}</div>`:''}`)}
...
function coverage(payload){const c=matchCardCoverage(payload),available=String(c.status).toUpperCase()==='AVAILABLE';const labels={MOTIVATION_UNAVAILABLE:'Мотивация',CONTEXT_UNAVAILABLE:'Контекст',PREDICTION_UNAVAILABLE:'Probability',MARKETS_UNAVAILABLE:'Рынки',ODDS_UNAVAILABLE:'Canonical odds',VALUE_RADAR_UNAVAILABLE:'Value Radar',CANONICAL_UNAVAILABLE:'Canonical'};return section('Coverage / качество данных',`<div class="match-card-v2-coverage ${available?'good':'partial'}"><strong>${available?'Карточка собрана полностью':'Часть разделов недоступна'}</strong>${c.limitations.length?`<div>${c.limitations.map(x=>badge(labels[x]||x,'warn')).join('')}</div>`:''}</div><div class="match-card-v2-safety">Read-only · без прямого provider polling · модель и eligibility не изменяются</div>`)}
...
export function renderMatchCard(payload={}){return `<div class="match-card-v2">${header(payload)}${motivation(payload)}${prediction(payload)}${markets(payload)}${odds(payload)}${context(payload)}${squad(payload)}${radar(payload)}${canonical(payload)}${coverage(payload)}</div>`}
