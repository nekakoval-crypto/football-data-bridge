import csv,io,json,math,os,re,time,gzip,urllib.parse,urllib.request,calendar
...
UA='football-data-bridge-stage43-v6/1.0'
def req_bytes(url,timeout=180,retries=5):
  last=None
  for i in range(retries):
    try:
      q=urllib.request.Request(url,headers={'User-Agent':UA});
      with urllib.request.urlopen(q,timeout=timeout) as r:return r.read()
    except Exception as e:last=e;time.sleep(min(10,1+2*i))
  raise last
def req_json(url,timeout=180,retries=5):return json.loads(req_bytes(url,timeout,retries).decode('utf-8'))
def decode(b):
...
def sim(a,b):return simnorm(norm(a),norm(b))
def pdate(s):
...
def multi_weather(start,end,entries):
  hourly='temperature_2m,relative_humidity_2m,precipitation,rain,snowfall,wind_speed_10m,wind_gusts_10m,weather_code'
  p={'latitude':','.join(f'{x[1]:.5f}' for x in entries),'longitude':','.join(f'{x[2]:.5f}' for x in entries),'start_date':start,'end_date':end,'hourly':hourly,'timezone':'auto','wind_speed_unit':'kmh','precipitation_unit':'mm','models':'era5_land'}
  try:d=req_json('https://archive-api.open-meteo.com/v1/archive?'+urllib.parse.urlencode(p),240,5)
  except:
    p.pop('models',None);d=req_json('https://archive-api.open-meteo.com/v1/archive?'+urllib.parse.urlencode(p),240,5)
...
by_club=defaultdict(list);by_stad=defaultdict(list)
for r,nc,ns in S:by_club[nc].append((r,nc,ns));by_stad[ns].append((r,nc,ns))
...
for league,home,tmhome,stadium in venue_keys:
  nh,nt,ns=norm(home),norm(tmhome),norm(stadium);cand=[]
  cand.extend(by_club.get(nt,[]));cand.extend(by_club.get(nh,[]));cand.extend(by_stad.get(ns,[]))
  if not cand:
    # shortlist by first token / containment before expensive similarity
    toks=set((nt+' '+nh+' '+ns).split())
    cand=[x for x in S if toks.intersection(set((x[1]+' '+x[2]).split()))]
  best=None;bs=-1;detail=''
  for r,nc,nstad in cand:
    cs=max(simnorm(nt,nc),simnorm(nh,nc));ss=simnorm(ns,nstad);score=max(cs,0.88*ss+0.12*cs)
    if score>bs:best=r;bs=score;detail=f'club={cs:.3f};stad={ss:.3f}'
  if best is not None and bs>=0.80:
    try:coords[(league,home)]={'lat':float(best['lat']),'lon':float(best['lon']),'source':'stadium_dataset_fuzzy','detail':best.get('club','')+' | '+best.get('name_of_stadium','')+' | '+detail};continue
    except:pass
...
with open('out_v6/stage43_team_coordinates.csv','w',encoding='utf-8-sig',newline='') as f:
...
with open('out_v6/stage43_coordinate_unmatched.csv','w',encoding='utf-8-sig',newline='') as f:
  w=csv.writer(f);w.writerow(['league','home','tm_home','stadium','best_score','best_club','best_stadium']);w.writerows(miss)
# month batches
bymonth=defaultdict(set)
for m in fd:
  k=(m['league'],m['home'])
  if k in coords:bymonth[m['date'][:7]].add(k)
tasks=[]
for ym,keys in sorted(bymonth.items()):
  y,mo=map(int,ym.split('-'));start=f'{y:04d}-{mo:02d}-01';end=f'{y:04d}-{mo:02d}-{calendar.monthrange(y,mo)[1]:02d}';ks=sorted(keys)
  for i in range(0,len(ks),12):
    b=ks[i:i+12];tasks.append((ym,start,end,[(k,coords[k]['lat'],coords[k]['lon']) for k in b]))
print('months',len(bymonth),'tasks',len(tasks),flush=True)
weather_by={};errors=[]
def task(t):
  ym,start,end,entries=t
  try:return ym,multi_weather(start,end,entries),''
  except Exception as e:return ym,{k:None for k,_,_ in entries},str(e)
with ThreadPoolExecutor(max_workers=10) as ex:
  fut=[ex.submit(task,t) for t in tasks]
  for n,fu in enumerate(as_completed(fut),1):
    ym,d,e=fu.result();
    for k,v in d.items():weather_by[(ym,k)]=v
    if e:errors.append((ym,e))
    if n%25==0 or n==len(tasks):print('api',n,'/',len(tasks),'errors',len(errors),flush=True)
out=[]
for m in fd:
  k=(m['league'],m['home']);data=weather_by.get((m['date'][:7],k));g=coords.get(k);z=dict(m);z.update({'weather_ok':0,'weather_error':''})
...
with open('out_v6/stage43_match_weather.csv','w',encoding='utf-8-sig',newline='') as f:
  w=csv.DictWriter(f,fieldnames=fields);w.writeheader();[w.writerow({k:r.get(k,'') for k in fields}) for r in out]
with open('out_v6/stage43_api_errors.csv','w',encoding='utf-8-sig',newline='') as f:
  w=csv.writer(f);w.writerow(['month','error']);w.writerows(errors)
