#!/usr/bin/env python3
"""Stage80 — descriptive PBK14 competition-congestion market research.

Input is the durable strict AUTO/HIGH PBK14 historical-market × PBK16 congestion join.

Research questions:
- does recent non-league load belong to the home side, away side, both or neither?
- UEFA vs domestic cup;
- <=72h and <=96h;
- previous non-league match on Thursday;
- Thursday -> weekend pattern.

Outputs descriptive outcome/goal/BTTS rates plus closing-market no-vig calibration
and flat-bet ROI for 1X2 and O/U2.5.

No factor is ranked, selected, promoted or given operational authority here.
"""
...
FACTORS=[
    ...
    "UEFA_72H_SIDE",
    "UEFA_96H_SIDE",
    "DOMESTIC_CUP_72H_SIDE",
    "DOMESTIC_CUP_96H_SIDE",
    "PREV_NONLEAGUE_THURSDAY_SIDE",
    "THURSDAY_TO_WEEKEND_SIDE",
]
...
def side_within(row,prefix,kind,hours):
    elapsed=fnum(row.get(f"{prefix}_hours_since_prev_nonleague"))
    if elapsed is None or not (0<elapsed<=hours):
        return False
    if kind=="ANY":
        return True
    if kind=="UEFA":
        return is_true(row.get(f"{prefix}_prev_nonleague_was_uefa"))
    if kind=="DOMESTIC_CUP":
        return is_true(row.get(f"{prefix}_prev_nonleague_was_domestic_cup"))
...
def match_weekday(row):
    raw=sval(row,"date_iso")
    try:
...
def factor_buckets(row):
    out={}
    for factor,kind,hours in (
        ("NONLEAGUE_72H_SIDE","ANY",72),
        ("NONLEAGUE_96H_SIDE","ANY",96),
        ("UEFA_72H_SIDE","UEFA",72),
        ("UEFA_96H_SIDE","UEFA",96),
        ("DOMESTIC_CUP_72H_SIDE","DOMESTIC_CUP",72),
        ("DOMESTIC_CUP_96H_SIDE","DOMESTIC_CUP",96),
    ):
        out[factor]=side_bucket(
            side_within(row,"home",kind,hours),
            side_within(row,"away",kind,hours),
        )

    home_thu=is_true(row.get("home_prev_nonleague_was_thursday"))
    away_thu=is_true(row.get("away_prev_nonleague_was_thursday"))
    out["PREV_NONLEAGUE_THURSDAY_SIDE"]=side_bucket(home_thu,away_thu)

    wd=match_weekday(row)
    if wd not in {6,7}:
        out["THURSDAY_TO_WEEKEND_SIDE"]="NOT_WEEKEND_FIXTURE"
    else:
        out["THURSDAY_TO_WEEKEND_SIDE"]=side_bucket(home_thu,away_thu)
    return out
...
def main():
    ...
    p.add_argument("--source",default="ops/pbk14_congestion_market_join_research.csv")
