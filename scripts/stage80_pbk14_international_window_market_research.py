...
international-window V2 join.

This layer asks only calendar-level questions that are supported by the source:
- is the domestic fixture BEFORE, AFTER or INSIDE the nearest UEFA-relevant window?
- how close is it to the window (<=72h, 72-96h, 96h-7d, farther)?
- which side is playing its first domestic league match after the same nearest window?
- what is the calendar window max-match allowance?

Player call-up, travel, appearance, minutes and return timing remain UNVERIFIED.
Outputs descriptive outcome/goal/BTTS rates plus closing-market no-vig
calibration and flat-bet ROI for 1X2 and O/U2.5.

No factor is ranked, selected, promoted or granted operational authority here.
"""
...
REFERENCE_CONTRACT="NEAREST_WINDOW_RELATION_GATED_V2"

FACTORS=[
    "WINDOW_RELATION",
    "BEFORE_WINDOW_BAND",
    "AFTER_WINDOW_BAND",
    "FIRST_DOMESTIC_AFTER_WINDOW_SIDE",
    "WINDOW_MAX_MATCHES",
]
...
def valid_row(row):
    return (
        ...
        and sval(row,"window_reference_contract")==REFERENCE_CONTRACT
        and sval(row,"window_relation") in {"BEFORE","AFTER","INSIDE"}
        ...
        and sval(row,"context_provider_calls") in {"","0"}
...
def distance_band(row,relation):
    actual=sval(row,"window_relation")
    if actual!=relation:
        return f"NOT_{relation}"
    if actual=="BEFORE":
        h=fnum(row.get("hours_to_window_start"))
    else:
        h=fnum(row.get("hours_since_window_end"))
    if h is None or h<0:
        return "UNKNOWN"
    if h<=72:
        return "WITHIN_72H"
...
def factor_buckets(row):
    max_matches=sval(row,"window_max_matches") or "UNKNOWN"
    return {
        "WINDOW_RELATION":sval(row,"window_relation"),
        "BEFORE_WINDOW_BAND":distance_band(row,"BEFORE"),
        "AFTER_WINDOW_BAND":distance_band(row,"AFTER"),
        "FIRST_DOMESTIC_AFTER_WINDOW_SIDE":side_bucket(
        ...
        "WINDOW_MAX_MATCHES":max_matches,
    }
...
def aggregate(joined):
    ...
    return [summarize(rows,*key) for key,rows in sorted(groups.items())]
...
def run(source,out_csv,stability_out,meta_out):
    ...
    meta={
        ...
        "window_reference_contract":REFERENCE_CONTRACT,
        ...
        "btts_note":"BTTS is reported as historical result rate only; no BTTS closing-market odds are normalized in this contour.",
