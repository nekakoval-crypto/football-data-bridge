...
class T(unittest.TestCase):
    def fx(self,fid,k,season="2025"):return {"fixture_id":str(fid),"country":"Norway","competition_name":"Eliteserien","season":season,"kickoff_utc":k,"status":"FT","home_team_id":"10","home_team":"A","away_team_id":"20","away_team":"B","home_goals":"2","away_goals":"1"}
    ...
    def test_ingestion_time_does_not_block_retrospective_history(self):
        f=[self.fx("1","2025-01-01T12:00:00Z"),self.fx("2","2025-02-01T12:00:00Z")]
        g=[self.g("1","2025-01-01T12:00:00Z",9,"2026-09-20T00:00:00Z"),self.g("2","2025-02-01T12:00:00Z",5,"2026-09-20T00:00:00Z")]
        rows,meta=m.build(g,f);r=[x for x in rows if x["fixture_id"]=="2"][0];self.assertEqual(r["season_sample"],1);self.assertEqual(r["last_grade"],9.0);self.assertTrue(meta["retrospective_reconstructed"]);self.assertFalse(meta["ingestion_observed_at_used_as_cutoff"])
