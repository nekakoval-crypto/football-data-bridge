...
class StandingsMotivationTests(unittest.TestCase):
    ...
    def test_registry_is_season_scoped_and_conservative(self):
        ...
        self.assertEqual(split['status'], 'VERIFIED')
        self.assertEqual(split['total_games'], 38)
        self.assertTrue(split['phase_aware'])
        self.assertEqual(split['regular_phase_games'], 33)
        historical_split = format_registry.get_format('179', '2025')
        self.assertEqual(historical_split['status'], 'UNKNOWN')
        self.assertIsNone(historical_split['total_games'])
