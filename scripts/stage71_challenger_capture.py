...
def main():
    ...
    bookmaker_id=BET365_ID; bet_id=MATCH_WINNER_BET_ID
    api_calls=0; warnings=[]; new_triggers=0; new_r1=0; new_r2=0; odds_calls=0; marathon_calls=0
    research_catalog=[r for r in catalog if r.get("league")!="Serie A"]
    for lg in research_catalog:
        lid=lg["api_league_id"]
        try: upcoming=next_fixture_rows(lid); api_calls+=1
        except Exception as e: warnings.append(f"{lg['league']}: fixtures-next error {e}"); upcoming=[]
        for x in upcoming:
            ...
            try:
                ...
                if bh and bd and ba:
                    ...
                    tr={"api_fixture_id":fid,"captured_at_utc":iso(now),"country":lg.get("country") or "","league":lg.get("league") or "","api_league_id":lid,"kickoff_utc":fx.get("date") or "","home_team_id":home.get("id") or "","home_team":home.get("name") or "","away_team_id":away.get("id") or "","away_team":away.get("name") or "","b365_home":bh,"b365_draw":bd,"b365_away":ba,"api_update_utc":upd,"source":"Stage71 first complete Bet365 1X2 capture"}
                    triggers.append(tr); trig_by_id[fid]=tr; new_triggers+=1
            except Exception as e: warnings.append(f"{lg['league']} fixture {fid}: Bet365 trigger error {e}")
    need_completed=set()
    for t in triggers:
        ko=parse_dt(t.get("kickoff_utc")); lid=str(t.get("api_league_id") or "")
        if lid and ko and ko>now and r1_from_trigger(t):need_completed.add(lid)
    for r in forward:
    ...
    for t in triggers:
        ...
        if not fid or not lid or not ko or ko<=now or not r1_from_trigger(t):continue
        hid=inum(t.get("home_team_id")); aid=inum(t.get("away_team_id")); (hp,hppg),(ap,appg)=team_state_before(completed_by_league.get(lid,[]),ko,hid,aid)
        hrs=(ko-now).total_seconds()/3600.0; r2_ok=hp>=5 and ap>=5 and hppg is not None and appg is not None and appg>hppg
        rid=f"R1|{lid}|{fid}"; rid2=f"R2|{lid}|{fid}"
        new_r1_needed=rid not in existing_ids
        new_r2_needed=hrs<=R2_FREEZE_HOURS and r2_ok and rid2 not in existing_ids
        if not new_r1_needed and not new_r2_needed:continue
        user_odd,_=marathon_away_price(fid,bet_id,t.get("home_team") or "",t.get("away_team") or "");api_calls+=1;marathon_calls+=1
        base={"country":t.get("country") or "","league":t.get("league") or "","api_league_id":lid,"api_fixture_id":fid,"kickoff_utc":t.get("kickoff_utc") or "","home_team_id":t.get("home_team_id") or "","home_team":t.get("home_team") or "","away_team_id":t.get("away_team_id") or "","away_team":t.get("away_team") or "","selection":"П2","stake_u":"1.000","trigger_captured_at_utc":t.get("captured_at_utc") or "","trigger_b365_home":t.get("b365_home") or "","trigger_b365_draw":t.get("b365_draw") or "","trigger_b365_away":t.get("b365_away") or "","home_played":hp,"away_played":ap,"home_last5_ppg":"" if hppg is None else f"{hppg:.3f}","away_last5_ppg":"" if appg is None else f"{appg:.3f}","user_bookmaker":"Marathonbet","user_odds":"" if user_odd is None else user_odd,"user_price_captured_at_utc":iso(now) if user_odd is not None else "","status":"PENDING","result":"","final_home_goals":"","final_away_goals":"","user_profit_u":"","settled_at_utc":""}
        if new_r1_needed:
            row=dict(base);row.update({"research_id":rid,"family":"R1","eligibility_frozen_at_utc":t.get("captured_at_utc") or "","notes":"research-only exact R1 analogue; never canonical automatically"});forward.append(row);existing_ids.add(rid);new_r1+=1
        if new_r2_needed:
            row=dict(base);row.update({"research_id":rid2,"family":"R2","eligibility_frozen_at_utc":iso(now),"notes":"research-only exact R2 analogue frozen close to kickoff; never canonical automatically"});forward.append(row);existing_ids.add(rid2);new_r2+=1
    ...
    META.write_text(json.dumps({"run_at_utc":iso(now),"status":"OK","research_leagues":len(research_catalog),"new_triggers":new_triggers,"new_r1_rows":new_r1,"new_r2_rows":new_r2,"settled_rows":settled,"trigger_rows":len(triggers),"forward_rows":len(forward),"api_calls":api_calls,"odds_calls":odds_calls,"marathon_calls":marathon_calls,"warnings":warnings},ensure_ascii=False,indent=2),encoding="utf-8")
