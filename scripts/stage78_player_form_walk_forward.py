...
def build(grades,fixtures):
    ...
    for rows in groups.values():
        ...
        for i,target in enumerate(rows):
            ...
            prior=[r for r in rows[:i] if str(r.get("kickoff_utc") or "")<tk and str((r.get("_fx") or {}).get("status") or "").upper() in {"FT","AET","PEN","FINISHED"} and num(r.get("overall_grade")) is not None]
            prior.sort(key=lambda r:str(r.get("kickoff_utc") or ""),reverse=True)
            if any(str(r.get("kickoff_utc") or "")>=tk for r in prior): leakage+=1; continue
            ...
            out.append({"fixture_id":target.get("fixture_id"),"kickoff_utc":tk,"country":f.get("country"),"competition_name":f.get("competition_name"),"season":f.get("season"),"team_id":target.get("team_id"),"team_name":oc["team_name"],"side":oc["side"],"player_id":target.get("player_id"),"player_name":target.get("player_name"),"position_group":target.get("position_group"),"sample_3":s3,"form_3":f3,"sample_5":s5,"form_5":f5,"sample_10":s10,"form_10":f10,"season_sample":len(sv),"season_mean":sm,"last_grade":lg,"trend_3_vs_10":round(f3-f10,4) if s3>=3 and s10>=10 else None,"deviation_last_vs_season":round(lg-sm,4) if lg is not None and sm is not None else None,"team_points":oc["points"],"team_goal_diff":round(oc["goal_diff"],4),"team_result":oc["result"],"prior_rows_max_kickoff_utc":max((str(r.get("kickoff_utc") or "") for r in prior),default=""),"prior_rows_max_observed_at_utc":max((str(r.get("observed_at_utc") or "") for r in prior),default=""),"evidence_time_basis":"PRIOR_TERMINAL_FIXTURE_CHRONOLOGY","retrospective_reconstructed":"true","no_lookahead":"true","research_only":"true","creates_signal":"false","probability_mutation":"false","eligibility_mutation":"false","stake_changes":"false","forward_journal_mutation":"false"})
    meta={"version":VERSION,"grade_rows":len(grades),"joined_grade_rows":joined,"output_rows":len(out),"form3_full_window_rows":sum(int(r["sample_3"])>=3 for r in out),"form5_full_window_rows":sum(int(r["sample_5"])>=5 for r in out),"form10_full_window_rows":sum(int(r["sample_10"])>=10 for r in out),"leakage_violations":leakage,"skipped_no_fixture":skipped_no_fixture,"skipped_no_team_side":skipped_no_side,"provider_calls":0,"research_only":True,"no_lookahead":True,"retrospective_reconstructed":True,"evidence_time_basis":"PRIOR_TERMINAL_FIXTURE_CHRONOLOGY","ingestion_observed_at_used_as_cutoff":False,"creates_signal":False,"probability_mutation":False,"eligibility_mutation":False,"stake_changes":False,"forward_journal_mutation":False}
    return out,meta
