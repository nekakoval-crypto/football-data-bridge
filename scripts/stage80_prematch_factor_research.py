#!/usr/bin/env python3
"""Stage80 — descriptive Top-5 prematch factor research.

Joins the reproducible Football-Data historical match matrix with the strict
no-lookahead prematch context projection and summarizes selected context buckets.

This is descriptive historical research only. Market no-vig probabilities are
market-implied baselines, never PBK probabilities. No factor row creates betting
...
def factor_buckets(row):
    ...
    if is_true(row.get("is_monday")):
        out["WEEKDAY"]="MONDAY"
    elif is_true(row.get("is_thursday")):
        out["WEEKDAY"]="THURSDAY"
    elif is_true(row.get("is_weekend")):
        out["WEEKDAY"]="WEEKEND"
    else:
        out["WEEKDAY"]="OTHER_WEEKDAY"

    mins=inum(row.get("kickoff_minutes_local"))
    if mins is None:
        out["KICKOFF_LOCAL"]="UNKNOWN"
    elif mins < 15*60:
        out["KICKOFF_LOCAL"]="BEFORE_15"
    elif mins < 18*60:
        out["KICKOFF_LOCAL"]="15_00_17_59"
    elif mins < 21*60:
        out["KICKOFF_LOCAL"]="18_00_20_59"
    else:
        out["KICKOFF_LOCAL"]="21_PLUS"

    hr=inum(row.get("home_rest_days")); ar=inum(row.get("away_rest_days"))
    if hr is None or ar is None:
        out["SHORT_REST"]="UNKNOWN"
        out["REST_ADVANTAGE"]="UNKNOWN"
    else:
        hs=hr<=3; ass=ar<=3
        if hs and ass:
            out["SHORT_REST"]="BOTH_SHORT_LE3D"
        elif hs:
            out["SHORT_REST"]="HOME_SHORT_ONLY"
        elif ass:
            out["SHORT_REST"]="AWAY_SHORT_ONLY"
        else:
            out["SHORT_REST"]="NEITHER_SHORT"

        diff=hr-ar
        if diff>=3:
            out["REST_ADVANTAGE"]="HOME_3PLUS"
        elif diff>=1:
            out["REST_ADVANTAGE"]="HOME_1_2"
        elif diff==0:
            out["REST_ADVANTAGE"]="EVEN_0"
        elif diff<=-3:
            out["REST_ADVANTAGE"]="AWAY_3PLUS"
        else:
            out["REST_ADVANTAGE"]="AWAY_1_2"

    h7=inum(row.get("home_matches_prev_7d")); a7=inum(row.get("away_matches_prev_7d"))
    if h7 is None or a7 is None:
        out["CONGESTION_7D_DIFF"]="UNKNOWN"
    elif h7>a7:
        out["CONGESTION_7D_DIFF"]="HOME_MORE"
    elif h7<a7:
        out["CONGESTION_7D_DIFF"]="AWAY_MORE"
    else:
        out["CONGESTION_7D_DIFF"]="EQUAL"

    hrank=inum(row.get("home_rank_pre")); arank=inum(row.get("away_rank_pre"))
    if hrank is None or arank is None:
        out["TABLE_RANK_DIFF"]="UNKNOWN"
    else:
        diff=hrank-arank
        if diff<=-5:
            out["TABLE_RANK_DIFF"]="HOME_5PLUS_BETTER"
        elif diff<=-1:
            out["TABLE_RANK_DIFF"]="HOME_1_4_BETTER"
        elif diff==0:
            out["TABLE_RANK_DIFF"]="EQUAL"
        elif diff<=4:
            out["TABLE_RANK_DIFF"]="AWAY_1_4_BETTER"
        else:
            out["TABLE_RANK_DIFF"]="AWAY_5PLUS_BETTER"

    out["FORM5_PPG_DIFF"]=ppg_bucket(
        row.get("home_ppg_last5"),row.get("away_ppg_last5"),
        row.get("home_form_matches_last5"),row.get("away_form_matches_last5")
    )
    out["VENUE_FORM5_PPG_DIFF"]=ppg_bucket(
        row.get("home_home_ppg_last5"),row.get("away_away_ppg_last5"),
        row.get("home_home_matches_last5"),row.get("away_away_matches_last5")
    )
    return out


def ppg_bucket(home,away,hmatches,amatches):
    h=fnum(home); a=fnum(away); hm=inum(hmatches); am=inum(amatches)
    if h is None or a is None or hm is None or am is None or hm<5 or am<5:
        return "UNKNOWN"
    diff=h-a
    if diff>=0.75:
        return "HOME_075PLUS_BETTER"
    if diff>=0.25:
        return "HOME_025_074_BETTER"
    if diff>-0.25:
        return "BALANCED"
    if diff>-0.75:
        return "AWAY_025_074_BETTER"
    return "AWAY_075PLUS_BETTER"
